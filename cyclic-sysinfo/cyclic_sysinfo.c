...
void gather_sysinfo()
{
    ...
    if (sysinfo(&info) == 0)
    {
        ...
        snprintf(message, MAX_MSG_SIZE + 1,
                 "uptime = %ld, total ram = %lu, free ram = %lu, process count "
                 "= %hu\n",
                 info.uptime, info.totalram, info.freeram, info.procs);
        send_to_mq(message);
        /* If you need to send this in the Line Protocol format, you'll format
         * it like: */
        /* sysinfo
         * uptime=<value>,total_ram=<value>,free_ram=<value>,process_count=<value>
         * <timestamp> */
    }
    else
    {
...
int main(int argc, char* argv[])
{
    while (1)
    {
        ...
        /* The sleep duration determines the 'cyclic' nature of the data
         * gathering */
        sleep(60); /* sleep for 1 second. Adjust this value as needed for your
                      use case */
    }
    return 0;
}
