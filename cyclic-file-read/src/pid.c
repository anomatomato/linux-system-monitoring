...
int pid_count() {
        ...
        pipe = popen("ps -e | wc -l", "r");                          /*anzahl der momentanen pids durch pipe ermitteln*/
...
int max_pids() {
        ...
        pipe = popen("cat /proc/sys/kernel/pid_max", "r");            /*anzahl der möglichen pids durch pipe ermitteln*/
        fgets(buffer, MAX_BUFFER, pipe);
        pclose(pipe);
...
int read_io_file(char* path, int line) {                  /*io-dateien sind geschützt, daher wird mit pipes gearbeitet*/
        FILE* pipe;
        int val;
        char buffer[MAX_BUFFER];
        ...
        sscanf(token, "%d", &val);                                    /*konvertierung, da addresse verloren gehen kann*/
        return val;
}
...
int write_pid_statm(char* path, char* id) {
        ...
        char statm_form[9][MAX_BUFFER] = {"proc-pid-statm,pid=\0", " size=\0", ",resident=\0",
                                          ",shared=\0", ",text=\0",
                                          ",lib=\0", ",data=\0", ",dt=\0", " \0"};
...
int pid(char* file) {
        ...
        for (int i = 0; i < amount; i++) {
                ...
                while ((check = write_pid_message(file, path, placeholder)) != 0) {           /*fürs protokol erfassen*/
                        if (check == 1) {                                     /*wenn eine pid nicht existiert ...*/
