...
enum _LP_Part { LP_MEASUREMENT, LP_TAG_KEY, LP_TAG_VALUE, LP_FIELD_KEY, LP_FIELD_VALUE };

/* Create a new key-value pair container */
static struct LP_Item *new_item(void) {
...
static void free_item(struct LP_Item *item) {
...
static int set_measurement(struct LP_Point *point, const char *line, size_t start, size_t end) {
        size_t i, j;
...
static int set_key(struct LP_Item *item, const char *line, size_t start, size_t end, enum _LP_Part part) {
        size_t i, j;
...
static int set_value(struct LP_Item *item, const char *line, size_t start, size_t end, enum _LP_Part part) {
        size_t i, j;
        item->type = LP_STRING;
...
static struct LP_Point *new_point(void) {
...
void LP_free_point(struct LP_Point *point) {
...
static size_t search_comma_space(const char *line, size_t start, size_t end, enum _LP_Part part) {
        size_t i = start;
...
static size_t search_equal(const char *line, size_t start, size_t end) {
        size_t i = start;
        for (; i < end; i++) {
...
static int parse_value(struct LP_Item *item) {
        size_t i = 0;
        size_t length = 0;
...
struct LP_Point *LP_parse_line(const char *line, int *status) {
