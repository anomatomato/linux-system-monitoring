...
static int on_systemd_signal(sd_bus_message* m, void* userdata,
                             sd_bus_error* ret_error)
{
    // Nutzdaten extrahieren
    const char *unit, *state;

    // Im realen Code müssten Sie die sd_bus_message parsen, um die notwendigen
    // Informationen zu extrahieren. Zum Beispiel so: int r =
    // sd_bus_message_read(m, "ss", &unit, &state); if (r < 0) {
    ...
    snprintf(formatted_message, sizeof(formatted_message),
             "systemd_event,unit=%s state=%s %ld", unit, state,
             (long int)time(NULL));
...
int main(int argc, char* argv[])
{
    ...
    ret = sd_bus_add_match(
        ...
        "freedesktop.systemd1.Manager',member='UnitNew'",
